...
### An SPK utility script does not start

Confirm that `node --version` is 20 or newer and `node` is on `PATH`, then run `/spk:doctor`. The default plugin install does not register an MCP server.
