---
name: start
description: Choose the smallest SPK workflow, or coordinate an explicitly requested product loop from vision and design through implementation and UAT.
---
...
4. Route by outcome:
   - visualize an idea, journey, or wireframe → `show-me`
   ...
   Default development to `code`. Choose `tdd` for explicit test-first requests or high-risk
   behavior with a reliable test seam. Let verification follow the change's risk and repo policy.
...
5. Run the selected workflow through its verified outcome. An explicit end-to-end request
   ...
   through planning and implementation, relevant checks, and repair of failures caused by the change
   without another local confirmation. Stop only at completion, a material user decision,
   or an unapproved effect. A first implementation alone is not completion.
6. Finish with the result first, then concise evidence, risks, and the next smallest action.

### Phase boundaries

Continue in the current conversation while the next phase needs its context. Read
[PHASE-BOUNDARIES.md](PHASE-BOUNDARIES.md) only when context pressure or an explicit session
transfer makes continuation impractical. Use host capabilities, not a fixed token threshold.
...
- Prefer one workflow for a narrow request; use the complete product loop when explicitly requested.
